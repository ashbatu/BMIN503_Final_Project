...
- Encounter type + timing are the most dominant features
- Note type and specialty are weaker (in terms of strength of effect)
- No predictors behave randomly 

**How other models compare**
- **Clinical Model**: Direction is stable for ~half of features; encounter type and specialty are the main drivers
- **'Other' Model**: Direction is stable for ~half of features; Encounter Type and time window are most stable and still main drivers
]
]
]
...
Main Takeaways
- LLMs can extract exclusion-relevant signals from EHR notes with high patient-level coverage 
- Note-level metadata can help with identifying which notes are more likely to contain exclusion-relevant information
...
- Note metadata **can** be used to prioritize chart reduce and reduce uncessary escreening effort

Ethical Considerations 
- Improves transparency in recruitment decisions
  - Especially for informal skip signals which can lead to bias
- Encourages consistent and explainable pre-screening logic

Limitations
- LLM predictions were not fully adjudicated
- Sample size was limited
- Specialty effects were very sparse
